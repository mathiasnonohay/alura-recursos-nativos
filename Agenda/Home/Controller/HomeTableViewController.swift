...
class HomeTableViewController: UITableViewController, UISearchBarDelegate, NSFetchedResultsControllerDelegate {
    ...
    @objc func abrirActionSheet(_ longPress:UILongPressGestureRecognizer) {
        if longPress.state == .began {
            ...
            let menu = MenuOpcoesAlunos().configuraMenuDeOpcoesDoAluno { (opcao) in
                switch opcao {
                case.sms:
                    if let componentMensagem = self.mensagem.configuraSMS(alunoSelecionado) {
                        componentMensagem.messageComposeDelegate = self.mensagem
                        self.present(componentMensagem, animated: true, completion: nil)
                    }
                    break
                case .ligacao:
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard let contadorDaListaDeAlunos = gerenciadorDeResultados?.fetchedObjects?.count else { return 0 }
